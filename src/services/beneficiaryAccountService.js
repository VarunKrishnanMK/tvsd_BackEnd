...
export async function getListBeneficiaryAccountDetails(req) {
    const userId = req.user;
    const accounts = await prisma.beneficiaryAccountDetails.findMany({ where: { userId: Number(userId) } });
...
export async function updateBeneficiaryAccountDetailsById(req) {
    ...
    const accounts = await prisma.beneficiaryAccountDetails.update({ where: { userId: Number(UId), id: Number(id) }, data: updatedData });
    if (!accounts) {
...
export async function deleteBeneficiaryAccountDetailsById(req) {
    ...
    await prisma.beneficiaryAccountDetails.delete({ where: { userId: Number(userId), id: Number(payload.id) } });
